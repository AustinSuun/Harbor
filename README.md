...
本地运行的多浏览器环境与任务管理工具。每个环境使用独立 Chromium 用户资料目录，支持手动登录、任务调度、历史记录和人工审核。

> **v0.2.0 预览版**：已完成 Windows 打包及静态检查，未执行功能测试或干净机器运行验收。网站页面变化可能影响自动化，请先小规模人工确认。Harbor 是独立工具，与 Arena.ai 无官方关联。
...
- 独立浏览器环境，最多同时运行 6 个环境；编辑不改变创建顺序。
- 鹈鹕动画、纯 HTML + CSS 网页复刻两种固定任务。
- 每轮任务数 1–50、间隔 3–3600 秒、并行数 1–3；并行名额覆盖发送及等待回复阶段。
- 任务记录、可选截图、人工评分、星标、标签、备注与导出。
- **不加载插件**，不提供三题数字任务或 Thinking 自动停止/归档功能。

任务需要在正确账号中手动确认登录，并勾选操作确认。停止任务仅停止管理器的调度与跟踪，不会撤回内容或自动停止网站正在生成的回复。不要使用本工具绕过网站限制；请遵守目标网站的使用条款及适用规则。
