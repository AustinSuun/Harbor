...
// Arena appends a date and/or a mode marker after the tier -- muse-spark-1-3-max-agent,
// deepseek-v4.1-flash-max-20260910 -- so the tier is the *last* tier word standing as its own
// segment, not necessarily the final segment. Anything after it is kept verbatim in `trailing`;
// nothing is inferred from it. Requiring at least one segment before the tier keeps `base`
// non-empty, so a bare tier-like name cannot match as a model.
export function parseModelLabel(label) {
  ...
  if (at > 0) {
    out.tier = segments[at];
    out.base = segments.slice(0, at).join('-');
    out.trailing = segments.slice(at + 1);
    out.date = out.trailing.find(s => DATE_RE.test(s)) || null;
  } else {
    const d = label.match(/-(\d{8}|\d{4})$/);
