...
async def main():
    ...
    base=dict(account_name='fixture',account_id='account',login_email='fixture@example.invalid',note='',profile_path='',auth_status='not_logged_in',task={'status':'idle'},trace_inspector={'enabled':False,'folder':'','runtime':'disabled_on_launch'})
    ...
    async with async_playwright() as pw:
        ...
        async def route(r):
            ...
            elif path=='/api/environments/kept/trace-inspector' and r.request.method=='PUT':
                assert r.request.headers.get('x-manager-request')=='1'
                data=json.loads(r.request.post_data);saved.append(data);envs[0]['trace_inspector'].update(data);data={'ok':True}
        ...
        try:
            ...
            button=page.get_by_role('button',name='Trace Inspector · 关闭',exact=True)
            await button.wait_for();assert await button.count()==1
            await button.click();assert not await page.locator('#trace-inspector-enabled').is_checked()
            assert await page.locator('#trace-inspector-folder').input_value()==''
            await page.locator('#trace-inspector-enabled').check()
            await page.locator('#trace-inspector-folder').fill('C:\\fixture\\Trace Inspector')
            await page.locator('#trace-inspector-save').click()
            await page.locator('#trace-inspector-editor').wait_for(state='hidden')
            assert saved==[{'enabled':True,'folder':'C:\\fixture\\Trace Inspector'}]
            await page.get_by_role('button',name='Trace Inspector · 已配置开启',exact=True).click()
            assert await page.locator('#trace-inspector-enabled').is_checked()
            await page.locator('#trace-inspector-enabled').uncheck();await page.locator('#trace-inspector-save').click()
            await page.locator('#trace-inspector-editor').wait_for(state='hidden')
